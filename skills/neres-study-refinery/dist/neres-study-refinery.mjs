#!/usr/bin/env node
import { createRequire as __neresCreateRequire } from "node:module"; const require = __neresCreateRequire(import.meta.url);
...
var DIAGRAM_MODES = ["auto", "off"];
var VISUAL_PROVIDERS = ["none", "agent-manifest", "openai"];
var VISUAL_CLASSIFICATIONS = [
  "textual-screenshot",
  "page-photo",
  "table",
  "diagram",
...
// src/errors.ts
var RefineryError = class extends Error {
  code;
  path;
...
// src/config.ts
var defaultConfigPath = path2.resolve(
  import.meta.dirname,
...
function rejectUnknownKeys(record2, section) {
  const allowed = new Set(ALLOWED_KEYS[section]);
...
function parseYaml(source, sourceLabel) {
  ...
  if (document.errors.length > 0) {
    throw new RefineryError(
      "ERR_CONFIG",
      `Configuration '${sourceLabel}' is invalid: ${document.errors[0]?.message ?? "parse error"}.`
    );
  }
  if (document.warnings.length > 0) {
    throw new RefineryError(
      "ERR_CONFIG",
      `Configuration '${sourceLabel}' has warnings: ${document.warnings[0]?.message ?? "warning"}.`
    );
  }
  ...
  for (const [key, value] of Object.entries(root)) {
    const section = key;
    const record2 = requireRecord(value, section);
    rejectUnknownKeys(record2, section);
  }
  return root;
}
function mergeConfiguration(base, override) {
  ...
  for (const [key, value] of Object.entries(override)) {
    ...
    merged[key] = isRecord(baseValue) && isRecord(value) ? { ...baseValue, ...value } : value;
  }
  return merged;
}
function requireBoolean(record2, key) {
...
function requireNumber(record2, key) {
  const value = record2[key];
...
function requireChoice(record2, key, choices) {
  const value = requireString(record2, key);
...
function requireStringArray(record2, key) {
  const value = record2[key];
...
function validateConfiguration(root) {
  ...
  return {
    ...
    output: {
      mode: requireChoice(output, "mode", ["separate-folder"]),
    ...
    diagrams: {
      ...
      provider: requireChoice(diagrams, "provider", ["archify"]),
...
async function readConfiguration(pathname) {
  try {
...
async function loadConfig(overridePath) {
  ...
  const absoluteOverride = path2.resolve(overridePath);
...
// src/scope/walk-scope.ts
import { readFile as readFile2, readdir, realpath as realpath2, stat as stat2 } from "node:fs/promises";
import path4 from "node:path";

// src/markdown/embeds.ts
var EMBED_PATTERN = /!\[\[([^\]]+)\]\]/gu;
...
// src/scope/boundary.ts
import { realpath, stat } from "node:fs/promises";
import path3 from "node:path";
function normalizeUserPath(value) {
  return value.replace(/[\\/]+/g, path3.sep);
}
...
function isSameOrDescendant(root, candidate) {
  const relative = path3.relative(comparisonPath(root), comparisonPath(candidate));
  return relative === "" || !relative.startsWith(`..${path3.sep}`) && relative !== ".." && !path3.isAbsolute(relative);
}
async function resolveVaultRoot(vault) {
  const lexical = path3.resolve(normalizeUserPath(vault));
  let resolved;
  try {
...
async function resolveExistingWithinVault(vaultRoot, requestedPath, missingCode = "ERR_INPUT_NOT_FOUND") {
  const normalized = normalizeUserPath(requestedPath);
  const lexical = path3.isAbsolute(normalized) ? path3.resolve(normalized) : path3.resolve(vaultRoot, normalized);
...
async function resolveOutputWithinVault(vaultRoot, requestedOutput) {
  ...
  const suffix = path3.relative(ancestor, lexical);
  return path3.resolve(resolvedAncestor, suffix);
}
function toVaultRelative(vaultRoot, absolutePath) {
  const relative = path3.relative(vaultRoot, absolutePath);
...
function kindForFile(filePath) {
  const extension = path4.extname(filePath).toLocaleLowerCase();
  if (extension === ".md") {
    return "markdown";
  }
...
function excludedNameSet(scope) {
  return new Set(
    scope.excludedDirectoryNames.map((name) => name.toLocaleLowerCase("en-US"))
  );
}
function comparisonPath2(value) {
  return process.platform === "win32" ? value.toLocaleLowerCase("en-US") : value;
}
function containsExcludedSegment(scope, absolutePath, excludedNames) {
  return path4.relative(scope.vaultRoot, absolutePath).split(path4.sep).some(
    (segment) => excludedNames.has(segment.toLocaleLowerCase("en-US"))
  );
}
function displayPath(scope, absolutePath) {
  if (isSameOrDescendant(scope.vaultRoot, absolutePath)) {
    return toVaultRelative(scope.vaultRoot, absolutePath);
  }
  return absolutePath.split(path4.sep).join("/");
}
async function collectEmbeddedImages(scope, notePath) {
  const markdown = await readFile2(notePath, "utf8");
  const entries = [];
  const rejected = [];
  for (const target of findObsidianEmbeds(markdown)) {
    const normalizedTarget = normalizeUserPath(target);
    const candidates = path4.isAbsolute(normalizedTarget) ? [path4.resolve(normalizedTarget)] : [
      path4.resolve(path4.dirname(notePath), normalizedTarget),
      path4.resolve(scope.vaultRoot, normalizedTarget)
    ];
    let resolved;
    let existsOutside = false;
    for (const candidate of candidates) {
      try {
        const candidateRealPath = await realpath2(candidate);
    ...
    if (!resolved) {
      rejected.push({
        path: normalizedTarget.split(path4.sep).join("/"),
        reason: existsOutside || path4.isAbsolute(normalizedTarget) ? "outside-vault" : "not-found"
      });
      continue;
    }
...
async function collectNote(scope) {
  const entries = [
    {
...
async function collectFolder(scope) {
  const entries = [];
  const rejected = [];
  const excludedNames = excludedNameSet(scope);
  const visitedDirectories = /* @__PURE__ */ new Set([
    comparisonPath2(scope.inputPath)
  ]);
  async function visit(directory, depth) {
    const children = await readdir(directory, { withFileTypes: true });
    children.sort((left, right) => comparePaths(left.name, right.name));
    for (const child of children) {
      const absolute = path4.join(directory, child.name);
      const relative = toVaultRelative(scope.vaultRoot, absolute);
      const lowerName = child.name.toLocaleLowerCase("en-US");
      if (child.isDirectory() || child.isSymbolicLink()) {
        if (excludedNames.has(lowerName) || isSameOrDescendant(scope.outputPath, absolute)) {
          rejected.push({ path: relative, reason: "excluded-directory" });
          continue;
        }
        let resolved2;
        try {
          resolved2 = await realpath2(absolute);
        } catch {
          rejected.push({ path: relative, reason: "not-found" });
          continue;
        }
        if (!isSameOrDescendant(scope.vaultRoot, resolved2)) {
          rejected.push({ path: relative, reason: "outside-vault" });
          continue;
        }
        if (!isSameOrDescendant(scope.inputPath, resolved2)) {
          rejected.push({ path: relative, reason: "outside-scope" });
          continue;
        }
        const metadata = await stat2(resolved2);
        if (!metadata.isDirectory()) {
          rejected.push({ path: relative, reason: "unsupported-type" });
          continue;
        }
        if (isSameOrDescendant(scope.outputPath, resolved2) || containsExcludedSegment(scope, resolved2, excludedNames) || visitedDirectories.has(comparisonPath2(resolved2))) {
          rejected.push({ path: relative, reason: "excluded-directory" });
          continue;
        }
        if (scope.includeSubfolders) {
          visitedDirectories.add(comparisonPath2(resolved2));
          await visit(resolved2, depth + 1);
        }
        continue;
      }
      if (!child.isFile()) {
...
async function collectScopeEntries(scope) {
  const collected = scope.inputType === "note" ? await collectNote(scope) : await collectFolder(scope);
  const uniqueEntries = /* @__PURE__ */ new Map();
  for (const entry of collected.entries) {
    uniqueEntries.set(entry.path.toLocaleLowerCase("en-US"), entry);
  }
  const entries = [...uniqueEntries.values()].sort(
    (left, right) => comparePaths(left.path, right.path)
  );
  const rejected = [...collected.rejected].sort(
    (left, right) => comparePaths(left.path, right.path)
  );
  return { entries, rejected };
}

// src/scope/resolve-scope.ts
import { stat as stat3 } from "node:fs/promises";
import path5 from "node:path";
function defaultOutput(inputPath, inputType) {
  if (inputType === "folder") {
    return path5.join(inputPath, "_V2");
  }
  const extension = path5.extname(inputPath);
  const stem = path5.basename(inputPath, extension);
  return path5.join(path5.dirname(inputPath), "_V2", `${stem}-V2.md`);
}
async function resolveScope(request, excludedDirectoryNames) {
  ...
  const metadata = await stat3(inputPath);
  if (request.inputType === "note") {
    if (!metadata.isFile() || path5.extname(inputPath).toLocaleLowerCase() !== ".md") {
      throw new RefineryError(
        "ERR_INPUT_TYPE",
  ...
  if (outputPath === inputPath || request.inputType === "note" && isSameOrDescendant(outputPath, inputPath) || request.inputType === "folder" && isSameOrDescendant(outputPath, inputPath)) {
    throw new RefineryError(
...
// src/planning/dry-run.ts
async function buildDryRunPlan(request, config) {
...
function formatDryRunPlan(plan) {
  ...
  for (const rejected of plan.rejectedEntries) {
    const label = rejected.reason === "excluded-directory" ? "Excluded" : "Rejected";
...
// src/transformation/build.ts
import { readFile as readFile7 } from "node:fs/promises";
import path14 from "node:path";

// src/classification/domain-profile.ts
var TERMS = {
  "law-afo": [
    "lei",
    "norma",
...
function scoreTerms(markdown, terms) {
...
function classifyDomainProfile(markdown, requestedProfile) {
...
// src/classification/source-state.ts
var STRUCTURED_HEADINGS = /* @__PURE__ */ new Set([
  "regras",
  "prazos",
  "edge cases",
  "defini\xE7\xF5es",
...
function classifySourceState(markdown, analysis) {
  const signals = [];
  let score = 0;
  ...
  if (analysis.headings.some(
    (heading) => STRUCTURED_HEADINGS.has(heading.text.trim().toLocaleLowerCase("pt-BR"))
  )) {
    score += 2;
...
var PRIMARY_HEADING = {
  "law-afo": "Vis\xE3o central",
  mathematics: "Ideia central",
  "technical-it": "Fun\xE7\xE3o central",
  hybrid: "Vis\xE3o integrada",
  generic: "Conte\xFAdo consolidado"
};
function sourceTitle(input) {
  return input.analysis.headings.find((heading) => heading.level === 1)?.text ?? path6.posix.basename(
    input.sourcePath,
    path6.posix.extname(input.sourcePath)
  );
}
function withoutFirstH1(markdown) {
  const normalized = markdown.replace(/\r\n?/gu, "\n").trim();
  return normalized.replace(/^#\s+.+?(?:\n+|$)/u, "").trim();
}
function traceability(sourcePath, model) {
  const rows = model?.claims.filter((claim) => claim.status === "supported").map((claim) => {
  ...
  return [
    ...
    "| Claim | Fonte | Local |",
    "|---|---|---|",
    ...rows.length > 0 ? rows : [`| \u2014 | [[${sourcePath}]] | documento |`]
  ].join("\n");
}
function annotateSupportedClaims(markdown, model) {
  if (!model) {
    return markdown;
  }
  ...
  for (const claim of model.claims) {
    ...
    annotated = annotated.slice(0, index) + `${marker}
` + annotated.slice(index);
  }
  return annotated;
}
function uncertaintySection(model) {
  if (!model || model.gaps.length === 0) {
    return void 0;
  }
  const callouts = model.gaps.map((gap) => {
    const region = gap.sourceRegion ? `, regi\xE3o \`${gap.sourceRegion}\`` : "";
    return [
      "> [!warning] Trecho visual incerto",
      `> A fonte \`${gap.sourcePath}\`${region} n\xE3o cont\xE9m evid\xEAncia leg\xEDvel suficiente.`
    ].join("\n");
  });
...
function visualEvidenceSection(input) {
  if (input.state !== "structured" || !input.model) {
    return void 0;
  }
  const excerpts = input.model.claims.filter(
    (claim) => claim.status === "supported" && claim.sourcePath !== input.sourcePath
  ).map((claim) => claim.sourceExcerpt).filter((value, index, values) => values.indexOf(value) === index);
  return excerpts.length > 0 ? `## Evid\xEAncias visuais
...
var SECTION_ORDER = {
  "law-afo": [
    "Vis\xE3o central",
    "Regras",
    "Estrutura ou composi\xE7\xE3o",
    "Compet\xEAncias e responsabilidades",
    "Condi\xE7\xF5es e requisitos",
    "Exce\xE7\xF5es e veda\xE7\xF5es",
    "Rela\xE7\xE3o com outros institutos",
    "Processos ou ciclos",
    "Defini\xE7\xF5es importantes",
  ...
  mathematics: [
    ...
    "Quest\xF5es das fontes"
  ],
  "technical-it": [
    "Fun\xE7\xE3o central",
    "Componentes",
    "Entradas e sa\xEDdas",
    "Fluxo de funcionamento",
    "Regras e restri\xE7\xF5es",
    "Implementa\xE7\xE3o ou sintaxe",
    "Exemplo t\xE9cnico",
    "Compara\xE7\xF5es",
    "Falhas e edge cases",
    "Revis\xE3o ativa"
  ...
  hybrid: [
    "Vis\xE3o integrada",
    "Regras",
    "F\xF3rmulas e c\xE1lculos",
    "Arquitetura e funcionamento",
    "Exemplos das fontes",
    "Pegadinhas e edge cases",
    "Revis\xE3o ativa"
  ],
  generic: ["Conte\xFAdo consolidado", "Exemplos das fontes", "Revis\xE3o ativa"]
};
function sectionForClaim(profile, type) {
  if (profile === "law-afo") {
    const mapping = {
      rule: "Regras",
      competence: "Compet\xEAncias e responsabilidades",
      condition: "Condi\xE7\xF5es e requisitos",
      exception: "Exce\xE7\xF5es e veda\xE7\xF5es",
      prohibition: "Exce\xE7\xF5es e veda\xE7\xF5es",
      classification: "Estrutura ou composi\xE7\xE3o",
      comparison: "Rela\xE7\xE3o com outros institutos",
      process: "Processos ou ciclos",
      definition: "Defini\xE7\xF5es importantes",
      "exam-trap": "Pegadinhas e edge cases",
      question: "Revis\xE3o ativa"
    };
    return mapping[type] ?? "Vis\xE3o central";
  }
  if (profile === "mathematics") {
    const mapping = {
      condition: "Quando usar",
      formula: "F\xF3rmula ou regra",
      rule: "F\xF3rmula ou regra",
      ...
      question: "Quest\xF5es das fontes"
    };
    return mapping[type] ?? "Ideia central";
  }
  if (profile === "technical-it") {
    const mapping = {
      classification: "Componentes",
      process: "Fluxo de funcionamento",
      rule: "Regras e restri\xE7\xF5es",
      condition: "Regras e restri\xE7\xF5es",
      prohibition: "Regras e restri\xE7\xF5es",
      code: "Implementa\xE7\xE3o ou sintaxe",
      example: "Exemplo t\xE9cnico",
      comparison: "Compara\xE7\xF5es",
      exception: "Falhas e edge cases",
      "exam-trap": "Falhas e edge cases",
      question: "Revis\xE3o ativa"
    };
    return mapping[type] ?? "Fun\xE7\xE3o central";
  }
  if (profile === "hybrid") {
    if (type === "formula") return "F\xF3rmulas e c\xE1lculos";
    ...
    if (type === "question") return "Revis\xE3o ativa";
    return "Vis\xE3o integrada";
  }
  if (type === "example" || type === "counterexample") {
    return "Exemplos das fontes";
  }
  if (type === "question") return "Revis\xE3o ativa";
  return "Conte\xFAdo consolidado";
}
function composeFromModel(input) {
...
// src/evidence/extract.ts
import path7 from "node:path";
function claimType(excerpt, forcedType) {
  if (forcedType) {
    return forcedType;
  }
...
function evidenceBlocks(markdown) {
  ...
  for (let index = 0; index < lines.length; ) {
    ...
    if (/^\s*(`{3,}|~{3,})/u.test(line)) {
      ...
      while (index < lines.length) {
        ...
        collected2.push(current);
        index += 1;
        if (new RegExp(`^\\s*${marker}{3,}\\s*$`, "u").test(current)) {
          break;
        }
      }
      blocks.push({ excerpt: collected2.join(newline), heading, forcedType: "code" });
      continue;
    }
    if (/^\s*>\s*\[![A-Za-z0-9_-]+\]/u.test(line)) {
      ...
      blocks.push({ excerpt: collected2.join(newline), heading });
      continue;
    }
    if (line.includes("|") && index + 1 < lines.length && /^\s*\|?(?:\s*:?-{3,}:?\s*\|)+/u.test(lines[index + 1] ?? "")) {
      const collected2 = [line, lines[index + 1] ?? ""];
      index += 2;
    ...
    index += 1;
    while (index < lines.length && (lines[index] ?? "").trim().length > 0 && !/^#{1,6}\s+/u.test(lines[index] ?? "") && !/^\s*(`{3,}|~{3,})/u.test(lines[index] ?? "") && !/^\s*>\s*\[![A-Za-z0-9_-]+\]/u.test(lines[index] ?? "")) {
    ...
    blocks.push({
      excerpt,
      heading,
      forcedType: /\$[^$]+\$|\$\$[\s\S]*\$\$/u.test(excerpt) ? "formula" : void 0
    });
  }
  return blocks.filter((block) => block.excerpt.trim().length > 0);
}
function routeClaims(claims, type) {
  return claims.filter((claim) => claim.type === type);
}
function extractContentModel(source, markdown, profile) {
  ...
  const topic = source.markdown.headings.find((heading) => heading.level === 1)?.text ?? path7.posix.basename(source.path, path7.posix.extname(source.path));
  return {
    topic,
    profile,
    claims,
...
// src/inventory/build-inventory.ts
import { createHash } from "node:crypto";
import { readFile as readFile3 } from "node:fs/promises";
import path8 from "node:path";

// src/markdown/analyze.ts
var TABLE_DELIMITER = /^\s*\|?(?:\s*:?-{3,}:?\s*\|)+\s*:?-{3,}:?\s*\|?\s*$/u;
var FENCE_START = /^\s*(`{3,}|~{3,})(.*)$/u;
...
function collectCodeBlock(lines, startIndex, marker, info) {
...
function collectCallout(lines, startIndex, type, title) {
  const collected = [lines[startIndex] ?? ""];
  let index = startIndex + 1;
  while (index < lines.length && /^\s*>/u.test(lines[index] ?? "")) {
    collected.push(lines[index] ?? "");
  ...
  return {
    callout: {
      ...
      content: collected.slice(1).map((line) => line.replace(/^\s*>\s?/u, "")).join("\n"),
      raw: collected.join("\n"),
      startLine: startIndex + 1,
      endLine: startIndex + collected.length
    },
...
function collectFormulas(line, lineNumber) {
  const formulas2 = [];
...
function normalizeRelative(value) {
  return path8.posix.normalize(value.replaceAll("\\", "/")).replace(/^\.\//u, "");
}
function embedCandidates(notePath, target) {
  const normalized = normalizeRelative(target);
  if (path8.posix.isAbsolute(normalized) || /^[A-Za-z]:\//u.test(normalized)) {
    return [];
  }
  return [
    normalizeRelative(path8.posix.join(path8.posix.dirname(notePath), normalized)),
    normalized
  ];
}
async function buildSourceInventory(scope, entries) {
  const sources = [];
  for (const [index, entry] of entries.entries()) {
    const bytes = await readFile3(entry.absolutePath);
    const source = {
  ...
  const imagesByPath = new Map(
    sources.filter((source) => source.type === "image").map((source) => [source.path.toLocaleLowerCase("en-US"), source])
  );
  for (const source of sources) {
    ...
    for (const embed of source.markdown.embeds) {
      for (const candidate of embedCandidates(source.path, embed)) {
        ...
        image.referencedBy = [.../* @__PURE__ */ new Set([...image.referencedBy ?? [], source.path])];
        break;
      }
    }
  }
...
// src/diagrams/archify-adapter.ts
import { spawn } from "node:child_process";
import { access, mkdir, readFile as readFile4, writeFile } from "node:fs/promises";

// src/diagrams/archify-input.ts
var NODE_KIND = "backend";
...
function prepareArchifyInput(candidate) {
  const base = {
    schema_version: 1,
  ...
  if (candidate.type === "lifecycle") {
    return {
      ...
      states: candidate.nodes.map((node, index) => ({
        id: node.id,
        type: index === 0 ? "start" : index === candidate.nodes.length - 1 ? "success" : "active",
        label: node.label,
        lane: "main",
        col: index
      })),
...
// src/diagrams/topology.ts
function decodeHtml(value) {
  return value.replace(/&quot;/gu, '"').replace(/&apos;|&#39;/gu, "'").replace(/&lt;/gu, "<").replace(/&gt;/gu, ">").replace(/&amp;/gu, "&").replace(
    /&#(\d+);/gu,
...
function validateRenderedTopology(candidate, html) {
  ...
  const renderedNodeContexts = /* @__PURE__ */ new Map();
  const renderedEdges = /* @__PURE__ */ new Map();
  for (const tag of canonicalSvg.match(/<[^>]+>/gu) ?? []) {
    const attrs = attributes(tag);
    const nodeId = attrs.get("data-node-id");
    if (nodeId) {
      const label = attrs.get("data-node-label");
      if (label === void 0) fail(`Rendered node '${nodeId}' has no label.`);
    ...
    if (edgeId) {
      ...
      if (existing && (existing.from !== value.from || existing.to !== value.to || existing.label !== value.label)) {
  ...
  for (const edge of candidate.edges) {
    ...
    if (!rendered || rendered.from !== edge.from || rendered.to !== edge.to || rendered.label !== edge.label) {
  ...
  for (const group of candidate.groups) {
    ...
    for (const nodeId of group.nodeIds) {
      ...
      const contexts = (renderedNodeContexts.get(nodeId) ?? "").split("\u203A").map((value) => value.trim());
...
// src/diagrams/archify-adapter.ts
function defaultRunner(executablePath) {
  return async (args) => await new Promise((resolve, reject) => {
    ...
    child.on("close", (code) => {
      ...
      reject(
        new RefineryError(
          "ERR_ARCHIFY",
          `Archify command failed with exit code ${String(code ?? 1)}${stderr.trim() ? `: ${stderr.trim().split(/\r?\n/u)[0]}` : ""}.`
        )
      );
    });
  });
}
function extractCanonicalSvg(html) {
...
var ArchifyAdapter = class {
  ...
  async render(candidate, outputDirectory) {
...
// src/diagrams/candidates.ts
import path9 from "node:path";
function slug(value) {
  return value.normalize("NFKD").replace(/[\u0300-\u036f]/gu, "").toLocaleLowerCase("en-US").replace(/[^a-z0-9]+/gu, "-").replace(/^-|-$/gu, "") || "diagram";
}
function typeFromVisual(classification) {
...
function candidateFromVisual(result, title) {
  if (!result.diagram || result.diagram.nodes.length === 0) {
    return void 0;
  }
  const base = path9.posix.basename(
    result.sourcePath,
    path9.posix.extname(result.sourcePath)
  );
  return {
    id: `${slug(base)}-diagram-01`,
    title,
    type: result.diagram.groups.length > 0 || result.diagram.nodes.length > 6 ? "architecture" : typeFromVisual(result.classification),
...
function numberedSteps(excerpt) {
  return excerpt.split(/\r?\n/u).map((line) => line.match(/^\s*\d+[.)]\s+(.+?)\s*$/u)?.[1]).filter((line) => Boolean(line));
}
function candidatesFromContentModel(model, sourcePath) {
  const candidates = [];
  ...
  for (const claim of model.processes) {
    ...
    candidates.push({
      id: `${slug(path9.posix.basename(sourcePath, ".md"))}-diagram-${String(
        candidateIndex
      ).padStart(2, "0")}`,
      title: model.topic,
      type: "workflow",
...
function scoreDiagramCandidate(candidate) {
  let score = 0;
  ...
  if (candidate.nodes.length >= 3 && candidate.edges.length >= candidate.nodes.length - 1) {
    score += 2;
  }
  if ([...outgoing.values()].some((count2) => count2 >= 2)) score += 2;
  if (candidate.type === "sequence") score += 2;
  if (candidate.type === "lifecycle") score += 2;
  if (candidate.nodes.some((node) => !incoming.has(node.id)) && candidate.nodes.some((node) => !outgoing.has(node.id))) {
    score += 1;
  }
  ...
  if (candidate.confidence < 0.85 || candidate.nodes.some((node) => node.confidence < 0.85) || candidate.edges.some((edge) => edge.confidence < 0.85)) {
    score -= 2;
  }
  return score;
}

// src/diagrams/detect-archify.ts
...
async function detectArchifyPath(explicitPath) {
  if (explicitPath) {
    return explicitPath;
  }
  ...
  const candidates = [
    configured,
    path10.join(homedir(), ".codex", "skills", "archify", "bin", "archify.mjs"),
    path10.join(homedir(), ".agents", "skills", "archify", "bin", "archify.mjs")
...
// src/images/create-extractor.ts
import path11 from "node:path";

// src/images/manifest-extractor.ts
import { readFile as readFile5 } from "node:fs/promises";

// src/images/validate-result.ts
...
function validateVisualPayload(value, provider, source) {
  ...
  if (!VISUAL_CLASSIFICATIONS.includes(classification)) {
    throw new RefineryError(
      "ERR_VISUAL_EXTRACTION",
...
// src/images/manifest-extractor.ts
var ManifestVisualExtractor = class _ManifestVisualExtractor {
  ...
  static async fromFile(filePath) {
    let parsed;
    try {
      parsed = JSON.parse(await readFile5(filePath, "utf8"));
    } catch (error) {
      throw new RefineryError(
        "ERR_VISUAL_MANIFEST",
    ...
    if (typeof parsed !== "object" || parsed === null || Array.isArray(parsed) || parsed.version !== 1 || !Array.isArray(parsed.sources)) {
      throw new RefineryError(
        "ERR_VISUAL_MANIFEST",
    ...
    const sources = parsed.sources.map((value) => {
      ...
      const source = value;
      if (typeof source.sourcePath !== "string" || typeof source.sourceSha256 !== "string" || !/^[a-f0-9]{64}$/u.test(source.sourceSha256)) {
        throw new RefineryError(
          "ERR_VISUAL_MANIFEST",
    ...
    return new _ManifestVisualExtractor(sources);
  }
  async extract(input) {
    const key = input.sourcePath.replaceAll("\\", "/").toLocaleLowerCase("en-US");
...
// src/images/openai-extractor.ts
...
function outputText(response) {
  ...
  for (const item of output) {
    ...
    for (const part of content) {
      if (typeof part === "object" && part !== null && part.type === "output_text" && typeof part.text === "string") {
        return part.text;
      }
    }
  }
...
var OpenAIVisualExtractor = class {
  provider = "openai";
  ...
  async extract(input) {
    ...
    const bytes = await readFile6(input.absolutePath);
    const request = {
      model: this.#model,
      store: false,
      input: [
        {
          ...
          content: [
            {
              type: "input_text",
              text: "Analyze only the visible pixels of this authorized study image. Do not use web, external facts, model knowledge, or inferred missing text. Preserve numbers, symbols, table cells, formula syntax, nodes, edges, labels, direction, and uncertainty. Use null or illegible instead of guessing."
            },
            {
              type: "input_image",
...
// src/images/create-extractor.ts
async function createVisualExtractor(request, scope) {
...
function imageMimeType(filePath) {
  const extension = path11.extname(filePath).toLocaleLowerCase("en-US");
...
// src/images/visual-evidence.ts
function supportedText(result) {
...
function appendTyped(model, claim) {
...
function appendVisualEvidence(model, result, minimumConfidence) {
  const content = supportedText(result);
  const supported = result.status === "supported" && result.confidence >= minimumConfidence && content !== void 0;
  const claim = {
    id: `claim-${String(model.claims.length + 1).padStart(3, "0")}`,
    type: content?.type ?? "statement",
    statement: supported ? content?.text ?? "" : "Trecho visual sem evid\xEAncia suficiente para afirma\xE7\xE3o factual.",
    sourceId: result.sourceId,
    sourcePath: result.sourcePath,
    sourceHeading: null,
    sourceExcerpt: content?.text ?? "[ileg\xEDvel]",
...
async function atomicWriteFile(target, content) {
  ...
  const temporary = path12.join(
    directory,
    `.${path12.basename(target)}.neres-tmp-${String(process.pid)}-${randomUUID()}`
  );
  try {
    await writeFile2(target === temporary ? `${temporary}.write` : temporary, content);
...
// src/output/frontmatter.ts
function yamlArray(values) {
  return JSON.stringify(values);
}
function addStudyFrontmatter(options) {
  const frontmatter = [
    "---",
    "type: study-note-v2",
...
// src/output/overview.ts
import path13 from "node:path";
function renderOverview(outputRoot, entries) {
  const collator2 = new Intl.Collator("pt-BR", {
    numeric: true,
    sensitivity: "base"
  });
  const sorted = [...entries].sort(
    (left, right) => collator2.compare(
      path13.relative(outputRoot, left.targetPath),
      path13.relative(outputRoot, right.targetPath)
    )
  );
  const links = sorted.map((entry) => {
    const relative = path13.relative(outputRoot, entry.targetPath).split(path13.sep).join("/").replace(/\.md$/iu, "");
...
function count(validations, key) {
  return validations.reduce((total, validation) => total + validation[key].length, 0);
}
function renderTransformationReport(data) {
  const supported = data.claims - count(data.validations, "grounding") - count(data.validations, "externalSources");
  const status = data.validations.every((validation) => validation.passed) ? "passed" : "failed";
  const profile = [...new Set(data.profiles)].join(", ") || "generic";
  return `# Relat\xF3rio de transforma\xE7\xE3o

## Escopo

- Entrada: ${data.input}
- Tipo: ${data.inputType}
- Perfil did\xE1tico: ${profile}
...
- F\xF3rmulas encontradas: ${String(data.formulasFound)}
- C\xF3digo encontrado: ${String(data.codeFound)}
...
- Tabelas reconstru\xEDdas: ${String(data.reconstructedTables)}
- F\xF3rmulas transcritas: ${String(data.transcribedFormulas)}
- Diagramas interpretados: ${String(data.interpretedDiagrams)}
- Trechos ileg\xEDveis: ${String(data.illegiblePassages)}
...
- Redu\xE7\xE3o estimada: ${data.estimatedReduction}
- Redund\xE2ncias removidas: ${String(data.redundanciesRemoved)}
- Estruturas preservadas: ${String(data.structuresPreserved)}

## Valida\xE7\xE3o

- Claims suportados: ${String(Math.max(0, supported))}
- Claims amb\xEDguos: 0
- Conflitos: 0
- Lacunas: ${String(count(data.validations, "grounding"))}
- N\xFAmeros sem fonte: ${String(count(data.validations, "numbers"))}
- Entidades sem fonte: ${String(count(data.validations, "entities"))}
- Altera\xE7\xF5es normativas: ${String(count(data.validations, "modality"))}
- F\xF3rmulas divergentes: ${String(count(data.validations, "formulas"))}
- C\xF3digo alterado: ${String(count(data.validations, "code"))}
- N\xF3s n\xE3o autorizados: 0
- Rela\xE7\xF5es n\xE3o autorizadas: 0
- Fontes externas: ${String(count(data.validations, "externalSources"))}

## Resultado

- Status: ${status}
- Motivo: ${status === "passed" ? "Todas as valida\xE7\xF5es obrigat\xF3rias passaram." : "Uma ou mais valida\xE7\xF5es obrigat\xF3rias falharam."}
`;
}
...
var MODALITY_PAIRS = [
  ["pode", "deve"],
  ["regra", "exce\xE7\xE3o"],
  ["facultativo", "obrigat\xF3rio"],
  ["necess\xE1rio", "suficiente"],
  ["antes", "depois"],
...
function folded(value) {
  return value.normalize("NFD").replace(new RegExp("\\p{Diacritic}", "gu"), "").toLocaleLowerCase("pt-BR");
}
function compareNumberEvidence(source, output) {
  const sourceNumbers = new Set(
    source.match(new RegExp("(?<![\\p{L}\\d])\\d+(?:[.,]\\d+)*(?:%|\\p{L}+)?", "gu")) ?? []
  );
  const outputNumbers = output.match(new RegExp("(?<![\\p{L}\\d])\\d+(?:[.,]\\d+)*(?:%|\\p{L}+)?", "gu")) ?? [];
  return unique2(outputNumbers.filter((value) => !sourceNumbers.has(value)));
}
function entities(value) {
  ...
  return unique2([...acronyms, ...explicit].filter(Boolean));
}
function compareEntityEvidence(source, output) {
...
function compareModalityEvidence(source, output) {
  ...
  for (const [leftOriginal, rightOriginal] of MODALITY_PAIRS) {
    ...
    if (hasWord(sourceFolded, left) && !hasWord(sourceFolded, right) && hasWord(outputFolded, right)) {
      changes.push(`${leftOriginal}\u2192${rightOriginal}`);
    }
    if (hasWord(sourceFolded, right) && !hasWord(sourceFolded, left) && hasWord(outputFolded, left)) {
...
function formulas(value) {
  const results = [];
  const withoutDisplay = value.replace(/\$\$([\s\S]*?)\$\$/gu, (_match, body) => {
  ...
  return unique2(results);
}
function compareFormulaEvidence(source, output) {
...
// src/validation/validate-output.ts
function validateOutput(options) {
  const factualOutput = options.model.claims.filter((claim) => claim.status === "supported").map((claim) => claim.sourceExcerpt).join("\n\n");
  const grounding = options.config.require_grounding ? validateGrounding(
    options.model,
    options.markdown,
    options.registeredSourcePaths
  ) : [];
  const unsupported = options.config.fail_on_unsupported_claim ? options.model.claims.filter(
    (claim) => claim.status !== "supported" && options.markdown.includes(`<!-- claimId: ${claim.id} -->`)
  ).map((claim) => `${claim.id}:unsupported`) : [];
  grounding.push(...unsupported);
  const externalSources = options.config.fail_on_external_source ? options.model.claims.filter((claim) => !options.registeredSourcePaths.has(claim.sourcePath)).map((claim) => claim.sourcePath) : [];
  const result = {
    grounding,
    numbers: options.config.verify_numbers ? compareNumberEvidence(options.sourceCorpus, factualOutput) : [],
    entities: options.config.verify_entities ? compareEntityEvidence(options.sourceCorpus, factualOutput) : [],
    modality: options.config.verify_modality ? compareModalityEvidence(options.sourceCorpus, factualOutput) : [],
    formulas: options.config.verify_formulas ? compareFormulaEvidence(options.sourceCorpus, factualOutput) : [],
    code: options.config.verify_code ? compareCodeEvidence(options.sourceCorpus, factualOutput) : [],
    externalSources,
    passed: false
  };
  result.passed = Object.entries(result).every(
    ([key, value]) => key === "passed" || value.length === 0
  );
  return result;
}
function assertOutputValidation(result) {
  if (result.passed) {
    return;
  }
  const summary = Object.entries(result).filter(
    ([key, value]) => key !== "passed" && Array.isArray(value) && value.length > 0
...
function diagramSection(markdown, candidates) {
...
function noteTarget(scopeInput, scopeOutput, sourcePath, inputType) {
  if (inputType === "note") {
    return scopeOutput;
  }
  const relative = path14.relative(scopeInput, sourcePath);
  const extension = path14.extname(relative);
  const stem = path14.basename(relative, extension);
  return path14.join(
    scopeOutput,
    path14.dirname(relative),
    `${stem}-V2.md`
  );
}
function auditDirectory(outputPath, targetPath, sourcePath, inputType) {
  const stem = path14.basename(sourcePath, path14.extname(sourcePath));
  if (inputType === "note") {
    return path14.join(path14.dirname(outputPath), "_audit", stem);
  }
  const targetRelative = path14.relative(outputPath, targetPath);
  return path14.join(
    outputPath,
    "_audit",
    path14.dirname(targetRelative),
    stem
  );
}
function inventorySource(inventory, sourcePath) {
...
async function writeOutputFile(target, content, createdFiles, vaultRoot) {
...
function isImageOnlyMarkdown(markdown) {
  return markdown.replace(/^---[\s\S]*?^---\s*$/mu, "").replace(/^#{1,6}\s+.*$/gmu, "").replace(/!\[\[[^\]]+\]\]/gu, "").replace(/!\[[^\]]*\]\([^)]*\)/gu, "").trim().length === 0;
}
function estimateReduction(sourceCharacters, outputCharacters) {
...
async function buildTransformation(request, config) {
  ...
  const visualResults = /* @__PURE__ */ new Map();
  if (extractor) {
  ...
  async function adapterForCandidates() {
    if (archifyChecked) {
  ...
  for (const entry of plan.entries) {
    ...
    const markdown = await readFile7(entry.absolutePath, "utf8");
    ...
    const relevantVisualResults = inventory.sources.filter(
      (candidate) => candidate.type === "image" && candidate.referencedBy?.includes(source.path)
    ).map((candidate) => visualResults.get(candidate.path)).filter((result) => result !== void 0);
    ...
    const candidates = request.diagrams === "off" ? [] : [
      ...relevantVisualResults.map((result) => candidateFromVisual(result, model.topic)).filter((candidate) => candidate !== void 0),
      ...candidatesFromContentModel(model, source.path)
    ].filter(
      (candidate) => candidate.confidence >= config.images.minimum_confidence && scoreDiagramCandidate(candidate) >= config.diagrams.minimum_score
    );
    const renderedCandidates = [];
    if (candidates.length > 0) {
      const adapter = await adapterForCandidates();
      if (adapter) {
        const assetsDirectory = path14.join(path14.dirname(target), "assets");
        for (const candidate of candidates) {
    ...
    const sourceCorpus = [
      markdown,
      ...relevantVisualResults.map(
        (result) => model.claims.filter(
          (claim) => claim.sourcePath === result.sourcePath && claim.status === "supported"
        ).map((claim) => claim.sourceExcerpt).join("\n")
      )
    ].join("\n\n");
    ...
    const sourceNotes = unique3(
      model.claims.filter(
        (claim) => inventory.sources.some(
          (candidate) => candidate.type === "markdown" && candidate.path === claim.sourcePath
        )
      ).map((claim) => claim.sourcePath)
    );
    const sourceImages = unique3(
      model.claims.filter(
        (claim) => inventory.sources.some(
          (candidate) => candidate.type === "image" && candidate.path === claim.sourcePath
        )
      ).map((claim) => claim.sourcePath)
    );
    const finalMarkdown = addStudyFrontmatter({
      markdown: body,
      profile: profile.profile,
      sourceScope: plan.scope.inputType === "note" ? source.path : toVaultRelative(plan.scope.vaultRoot, plan.scope.inputPath),
      sourceNotes,
      sourceImages,
      compression: request.compression,
    ...
    await writeOutputFile(
      path14.join(audit, "source-inventory.json"),
      stableJson(inventory),
      createdFiles,
      plan.scope.vaultRoot
    );
    await writeOutputFile(
      path14.join(audit, "content-model.json"),
      stableJson(model),
      createdFiles,
      plan.scope.vaultRoot
    );
    await writeOutputFile(
      path14.join(audit, "classification.json"),
    ...
    if (relevantVisualResults.length > 0) {
      await writeOutputFile(
        path14.join(audit, "visual-evidence.json"),
  ...
  if (plan.scope.inputType === "folder" && config.output.create_overview) {
    await writeOutputFile(
      path14.join(plan.scope.outputPath, "_Vis\xE3o Geral.md"),
  ...
  if (config.output.create_audit_report) {
    const reportRoot = plan.scope.inputType === "note" ? path14.dirname(plan.scope.outputPath) : plan.scope.outputPath;
    const inputName = path14.basename(
      plan.scope.inputPath,
      path14.extname(plan.scope.inputPath)
    );
    ...
    const reportData = {
      ...
      ignoredExternalLinks: inventory.sources.filter((source) => source.markdown).reduce(
        (total, source) => total + (source.markdown?.links.filter((link) => /^https?:\/\//iu.test(link)).length ?? 0),
        0
      ),
      rawNotes,
      structuredNotes,
      ...
      notesV2: noteCount,
      diagramsGenerated,
      tablesCreated: tablesFound + visualValues.filter((result) => result.markdownTable).length,
...
function buildCommand(config, io) {
  const program2 = new Command();
  program2.name("neres-study-refinery").description(
  ...
  const build = program2.command("build").description("Plan or build a closed-source study-note transformation.").requiredOption("--vault <path>", "Obsidian vault root.").requiredOption("--input <path>", "Authorized note or folder inside the vault.").addOption(
    new Option("--input-type <type>", "Input kind.").choices([...INPUT_TYPES]).makeOptionMandatory()
  ).addOption(
    new Option(
      "--include-subfolders <boolean>",
      "Include nested folders (true or false)."
    ).argParser(parseBoolean).default(config.input.include_subfolders)
  ).addOption(
    new Option("--profile <profile>", "Didactic profile.").choices([...PROFILES]).default(config.classification.profile)
  ).option("--output <path>", "Separate output note or folder.").addOption(
    new Option("--compression <mode>", "Compression mode.").choices([...COMPRESSION_MODES]).default(config.transformation.compression)
  ).addOption(
    new Option("--diagrams <mode>", "Diagram planning mode.").choices([...DIAGRAM_MODES]).default(config.diagrams.mode)
  ).addOption(
    new Option("--visual-provider <provider>", "Visual extraction provider.").choices([...VISUAL_PROVIDERS]).default("none")
  ).option(
    ...
    false
  ).option("--openai-model <model>", "Explicit OpenAI multimodal model.").option(
    "--archify-path <path>",
...
async function runCli(argv, io = { stdout: process.stdout, stderr: process.stderr }) {
  try {
    const config = await loadConfig(configPathFromArgv(argv));
    const program2 = buildCommand(config, io);
