...
import { validateBundle } from "./opencode-bundle.mjs";
...
try {
  ...
  const result = await validateBundle({ bundleRoot, modelIds });
  if (!result.valid) {
    process.stderr.write(`${result.diagnostics.join("\n")}\n`);
    process.exitCode = 1;
  } else if (options.json) {
    process.stdout.write(`${JSON.stringify({ version, agents: result.agents }, null, 2)}\n`);
  } else {
    process.stdout.write(`Validated ${result.agents.length} agents for OpenCode ${version}.\n`);
  }
} catch (error) {
